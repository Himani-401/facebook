
document.addEventListener("DOMContentLoaded", function() {
    var leftImage = document.querySelector(".left");
    var settingsMenu = document.querySelector(".settings-menu");

    leftImage.addEventListener("click", function() {
        settingsMenu.classList.toggle("open-menu");
    });
});



dark=document.getElementById("dark_btn");
dark.onclick=function(){
    dark.classList.toggle("dark_btn_on");
    document.body.classList.toggle("dark-theme");
}




const likeButtons = document.querySelectorAll('.like-btn');
    const likeCounts = document.querySelectorAll('.like-count');
    
    likeButtons.forEach((button, index) => {
      let count = parseInt(likeCounts[index].textContent);
      let isLiked = false;
      
      button.addEventListener('click', function() {
        if (!isLiked) {
          count++;
          isLiked = true;
        } else {
          count--;
          isLiked = false;
        }
        
        likeCounts[index].textContent = count;
      });
    });


    async function fetchRandomText() {
        try {
            const response = await fetch('https://baconipsum.com/api/?type=all-meat&sentences=1');
            const data = await response.json();
            return data[0]; // Return the first sentence from the response
        } catch (error) {
            console.error('Error fetching random text:', error);
            return 'Lorem ipsum dolor sit amet.'; // Return a default text in case of error
        }
    }

    async function generateRandomPosts(numPosts) {
        const postsContainer = document.getElementById('posts-container');

        for (let i = 0; i < numPosts; i++) {
            const randomText = await fetchRandomText();

            const postElement = document.createElement('div');
            postElement.classList.add('post');
            postElement.innerHTML = `
                <p>${randomText}</p>
                <p><img src="https://source.unsplash.com/random/300x200" alt="Random Image"></p>
            `;
            postsContainer.appendChild(postElement);
        }
    }

    generateRandomPosts(1); 


     // Function to open the custom dialog
     function openCustomDialog() {
        document.getElementById('custom-dialog-overlay').style.display = 'flex';
    }

    // Function to close the custom dialog
    function closeCustomDialog() {
        document.getElementById('custom-dialog-overlay').style.display = 'none';
    }

    // Function for custom action - Twitter
    function customActionTwitter() {
        alert('Share on Twitter');
    }

    // Function for custom action - WhatsApp
    function customActionWhatsApp() {
        alert('Share on WhatsApp');
    }

    // Function for custom action - LinkedIn
    function customActionLinkedIn() {
        alert('Share on LinkedIn');
    }

    // Function for custom action - Mail
    function customActionMail() {
        alert('Share via Email');
    }



    function openImageDialog(imageSrc) {
        // Create dialog elements
        const dialogOverlay = document.createElement('div');
        dialogOverlay.classList.add('dialog-overlay');
        dialogOverlay.addEventListener('click', closeDialog); // Close dialog when clicked outside

        const dialogContent = document.createElement('div');
        dialogContent.classList.add('dialog-content');

        const imageElement = document.createElement('img');
        imageElement.classList.add('dialog-image');
        imageElement.src = imageSrc;

        // Append elements to the dialog
        dialogContent.appendChild(imageElement);
        dialogOverlay.appendChild(dialogContent);

        // Append dialog to the document body
        document.body.appendChild(dialogOverlay);

        // Display the dialog
        dialogOverlay.style.display = 'flex';

        // Function to close the dialog
        function closeDialog() {
            dialogOverlay.removeEventListener('click', closeDialog);
            dialogOverlay.remove();
        }
    }

    // Event listener for the open dialog button
    document.getElementById('open-dialog-button').addEventListener('click', function() {
        // Replace 'imageSrc' with the actual URL of your image
        openImageDialog('data:image/jpeg;base64,')
    });

    function openVideoDialog(videoSrc) {
        // Create dialog elements
        const overlay = document.createElement('div');
        overlay.classList.add('custom-overlay');
        overlay.addEventListener('click', closeDialog); // Close dialog when clicked outside

        const content = document.createElement('div');
        content.classList.add('custom-content');

        const video = document.createElement('video');
        video.classList.add('custom-video');
        video.src = videoSrc;
        video.controls = true; // Add controls to the video

        // Append elements to the dialog
        content.appendChild(video);
        overlay.appendChild(content);

        // Append dialog to the document body
        document.body.appendChild(overlay);

        // Display the dialog
        overlay.style.display = 'flex';

        // Function to close the dialog
        function closeDialog() {
            overlay.removeEventListener('click', closeDialog);
            overlay.remove();
        }
    }

    // Event listener for the image click
    document.getElementById('thumbnail').addEventListener('click', function() {
        // Replace 'videoSrc' with the actual URL of your video
        openVideoDialog('https://player.vimeo.com/progressive_redirect/playback/914161338/rendition/540p/file.mp4?loc=external&oauth2_token_id=1747418641&signature=c0ecd6dc733277d2d983c3ffac1fca6d507859d14e4ecc11bb60db58b0a143ca');
    });
    